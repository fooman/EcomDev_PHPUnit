...
class EcomDev_PHPUnit_Model_Fixture_Processor_EntityType implements EcomDev_PHPUnit_Model_Fixture_ProcessorInterface
{
    const STORAGE_KEY = 'entity_type';
    ...
    public function apply(array $data, $key, EcomDev_PHPUnit_Model_FixtureInterface $fixture)
    {
        ...
        foreach ($data as $entityType) {
            ...
            $object = Mage::getModel('eav/entity_type')->setData($entityType);
            ...
            $object->save();
            if ($object->getId()) {
                $entityTypes[self::STORAGE_KEY][$object->getId()] = $object;
            }
        }
        EcomDev_PHPUnit_Test_Case_Util::app()->getCache()->clean();
        Mage::getSingleton('eav/config')->clear();

        $fixture->setStorageData(self::STORAGE_KEY, $entityTypes);
